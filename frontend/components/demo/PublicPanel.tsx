  ...
  return (
    <section className="w-full md:w-[45%] p-8 flex flex-col relative bg-ink text-paper min-h-[600px]">
      ...
      <div className="flex-1 flex flex-col justify-center align-left max-w-md w-full mx-auto md:ml-12 md:mr-8 px-10">
        {/* Verification Input */}
        <div className="mb-12">
          <div className="border-b-2 border-line pb-2 focus-within:border-neon transition-colors mb-2">
            ...
            <input
              ...
              placeholder="Enter Stellar address (G...) or select"
            />
            <datalist id="address-options">
              {config.sampleAddr && (
                <option value={config.sampleAddr}>Pre-filled Sample Address</option>
              )}
            </datalist>
          </div>

          <p className="mt-2 text-xs font-mono text-muted">
            *You can select the sample address or type your own.
          </p>
